...
def save_result(modelled: ModelledDataset, results_root: str):
    ...
    np.save(fname('transformed_dataset.npy'), modelled.transformed_dataset)
    save(fname('full_result.pkl'), modelled)


if __name__ == '__main__':
    arguments = Arguments(*sys.argv[1:5])
    ...
    print(f'Merged {len(datasets)} datasets with '
          f'{merged_dataset.spectra.shape[0]} spectra.')
    number_of_components = int(.02 * merged_dataset.spectra.shape[1])
